...
func (a *App) handleOpenAICompatibility(w http.ResponseWriter, r *http.Request) {
	...
	case "/config", "/config/update":
		...
		writeJSON(w, http.StatusOK, map[string]any{
			"ENABLE_OPENAI_API":    a.config.OpenAIBaseURL != "",
			"OPENAI_API_BASE_URLS": []string{a.config.OpenAIBaseURL},
			"OPENAI_API_KEYS":      []string{a.config.OpenAIAPIKey},
			"OPENAI_API_CONFIGS":   map[string]any{},
		})
	case "/urls":
		writeJSON(w, http.StatusOK, map[string]any{"OPENAI_API_BASE_URLS": []string{a.config.OpenAIBaseURL}})
	case "/keys":
		writeJSON(w, http.StatusOK, map[string]any{"OPENAI_API_KEYS": []string{a.config.OpenAIAPIKey}})
	...
	default:
		a.proxyProvider(w, r, a.config.OpenAIBaseURL, a.config.OpenAIAPIKey, path)
	}
}
