...
func (a *App) handleResourceList(kind string, activeOnly bool) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		for _, resource := range resources {
			if !resourceReadableBy(resource, user, kind) {
				continue
			}
...
func resourceReadableBy(resource store.Resource, user store.User, kind string) bool {
	...
	access, exists := body["access_control"]
	if !exists || access == nil {
		return true
	}
	rules, _ := access.(map[string]any)
	if len(rules) == 0 {
		return false
	}
	for _, permission := range []string{"read", "write"} {
		entry, _ := rules[permission].(map[string]any)
		ids, _ := entry["user_ids"].([]any)
		for _, id := range ids {
			if id == user.ID {
				return true
			}
...
func (a *App) handlePromptByCommand(operation string) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		user, ok := a.requireUser(w, r)
		if !ok {
			return
		}
		command := strings.TrimPrefix(strings.TrimSpace(r.PathValue("command")), "/")
		resource, err := a.store.ResourceByKey(r.Context(), "prompt", command)
		...
		if operation == "get" {
			if !resourceReadableBy(resource, user, "prompt") {
		...
		switch operation {
		case "delete":
			if err := a.store.DeleteResource(r.Context(), "prompt", resource.ID); err != nil {
				writeError(w, http.StatusInternalServerError, "failed to delete prompt")
				return
			}
			writeJSON(w, http.StatusOK, true)
		case "toggle":
			updated, err := a.store.ToggleResource(r.Context(), "prompt", resource.ID)
			if err != nil {
				writeError(w, http.StatusInternalServerError, "failed to toggle prompt")
				return
			}
		...
		case "update":
			...
			resource.Key = resourceKey("prompt", current, resource.ID)
			resource.Body, _ = json.Marshal(current)
			updated, err := a.store.PutResource(r.Context(), resource)
			if err != nil {
				writeError(w, http.StatusBadRequest, "failed to update prompt")
				return
			}
			writeRawJSON(w, http.StatusOK, resourceResponse(updated))
		}
...
func (a *App) handlePromptMeta(w http.ResponseWriter, r *http.Request) {
	user, ok := a.requireUser(w, r)
	if !ok {
		return
	}
	resource, err := a.store.ResourceByID(r.Context(), "prompt", r.PathValue("id"))
	if errors.Is(err, store.ErrResourceNotFound) {
		writeError(w, http.StatusNotFound, "Prompt not found")
		return
	}
	...
	updated, err := a.store.PutResource(r.Context(), resource)
	if err != nil {
		writeError(w, http.StatusInternalServerError, "failed to update prompt metadata")
		return
	}
	writeRawJSON(w, http.StatusOK, resourceResponse(updated))
}
...
func (a *App) handleResourceByID(kind, operation string) http.HandlerFunc {
	return func(response http.ResponseWriter, request *http.Request) {
		...
		if operation == "get" && resourceReadableBy(resource, user, kind) {
			// Readable shared resources retain owner-only mutation semantics below.
		} else if resource.UserID != user.ID && user.Role != "admin" {
