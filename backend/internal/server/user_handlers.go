...
func (a *App) handleCurrentUserSettings(response http.ResponseWriter, request *http.Request) {
	...
	currentRaw, err := a.store.UserSettings(request.Context(), user.ID)
	if err != nil {
		writeError(response, http.StatusInternalServerError, "failed to load settings")
		return
	}
	...
	if value, ok := current["revision"].(float64); ok {
		currentRevision = int64(value)
	}
	if expected, present := patch["revision"].(float64); present && int64(expected) != currentRevision {
		writeError(response, http.StatusConflict, "User settings were updated elsewhere. Please retry with the latest settings.")
		return
	}
	delete(patch, "revision")
	merged := mergeSettingsMap(current, patch, nil)
	merged["revision"] = currentRevision + 1
	encoded, err := json.Marshal(merged)
	if err != nil {
		writeError(response, http.StatusBadRequest, "invalid settings")
		return
	}
	updated, err := a.store.SetUserSettings(request.Context(), user.ID, encoded)
	if err != nil {
...
func mergeSettingsMap(current, patch map[string]any, path []string) map[string]any {
	...
	for key, value := range patch {
		...
		if len(nextPath) == 2 && nextPath[0] == "ui" && nextPath[1] == "connections" {
			merged[key] = value
			continue
		}
		...
		if patchOK && currentOK {
			merged[key] = mergeSettingsMap(currentMap, patchMap, nextPath)
			continue
		}
		merged[key] = value
	}
